...
assertEquals(progress.getEntry(initial, "P001").attempts, 0, "new scenario attempts should default to 0")
assertEquals(progress.getEntry(initial, "P001").solved, false, "new scenario solved should default to false")

progress.applyResult({ id = "P001", solved = false, attempts = 1 })
local saved = progress.load()
assertEquals(saved.scenarios.P001.attempts, 1, "first local attempt should save")
assertEquals(saved.scenarios.P001.solved, false, "failed attempt should remain unsolved")
...
cloudFile:write('return {version=1,scenarios={P001={attempts=4,solved=true},P002={attempts=2,solved=false}}}\n')
cloudFile:close()

local synced = progress.load()
assertEquals(synced.scenarios.P001.attempts, 4, "load should pick up externally synced attempts")
assertEquals(synced.scenarios.P001.solved, true, "load should pick up externally synced solved state")
assertEquals(synced.scenarios.P002.attempts, 2, "load should pick up additional externally synced scenario")
