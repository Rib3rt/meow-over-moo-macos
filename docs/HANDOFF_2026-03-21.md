...
Update: 2026-04-23
- Turn phase confirmation UX is now aligned for local human modes:
  - `SINGLE_PLAYER`: manual phase button only for `confirmCommandHub`.
  - `MULTYPLAYER_LOCAL`: same policy as single player.
  - setup/deployment/actions phase confirmations are auto-accepted.
- `MULTYPLAYER_NET` keeps the same automatic phase flow already active (no behavior regression intended).
- Linux packaging/operator notes clarified:
  - in WSL prefer `bash MAKE_LINUX_PACKAGE.sh` / `bash MAKE_LINUX_PACKAGE_RELEASE.sh`
  - Steam depot upload root must be the extracted contents of `game/` (do not upload a zip as content root).
